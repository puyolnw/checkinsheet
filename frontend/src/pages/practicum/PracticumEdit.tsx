...
const PracticumEdit: React.FC = () => {
  ...
  const fetchRecord = async () => {
    try {
      setLoadingData(true);
      const response = await practicumAPI.getById(id!);
      ...
      if (response.success) {
        ...
        setFormData({
          ...
          activities_description: record.activities_description || '',
          learning_outcomes: record.learning_outcomes || '',
          challenges_faced: record.challenges_faced || '',
          reflections: record.reflections || '',
          status: record.status || 'draft'
        });
      } else {
        setError('ไม่พบข้อมูลบันทึกการฝึกปฏิบัติ');
      }
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const response = await practicumAPI.update(id!, {
      ...
      if (response.success) {
        ...
        setError(response.message || 'เกิดข้อผิดพลาดในการอัปเดตบันทึกการฝึกปฏิบัติ');
      }
    } catch (err: any) {
      setError(err.response?.data?.message || 'เกิดข้อผิดพลาดในการอัปเดตบันทึกการฝึกปฏิบัติ');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">แก้ไขบันทึกการฝึกปฏิบัติ</h1>
        <p className="text-gray-600">แก้ไขข้อมูลบันทึกการฝึกประสบการณ์วิชาชีพครู</p>
      </div>
